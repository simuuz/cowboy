...
void Cpu::step() {
    ...
    if(!halt) {
        u8 opcode = bus.next_byte(regs.pc, regs.pc);
...
void Cpu::execute(u8 opcode) {
    switch(opcode) {
        ...
        write_r16<1>((opcode >> 4) & 3, bus.next_word(regs.pc, regs.pc));
        break;
        ...
        break;
        case 0x08:                                    //LD (u16), SP
        bus.write_word(bus.next_word(regs.pc, regs.pc), regs.sp);
        break;
        case 0xf9:                                    //LD SP, HL
        regs.sp = regs.hl;
        break;
        case 0xe8: {                                  //ADD SP, i8
            u8 offset = bus.next_byte(regs.pc, regs.pc);
            bool z = false;
            bool n = false;
        ...
        break;
        case 0xd6: {                                  //SUB u8
            u8 op2 = bus.next_byte(regs.pc, regs.pc);
        ...
        halt = true;
        break;
        case 0xc4: case 0xd4: case 0xcc: case 0xdc: case 0xcd: { //CALL cond u16
            u16 addr = bus.next_word(regs.pc, regs.pc);
            if(cond(opcode)) {
                push(regs.pc);
                regs.pc = addr;
                cycles += 12;
        ...
        break;
        case 0xea:                                    //LD (u16), A
        bus.write_byte(bus.next_word(regs.pc, regs.pc), regs.a);
        break;
        case 0xfa:                                    //LD A, (u16)
        regs.a = bus.read_byte(bus.next_word(regs.pc, regs.pc));
        break;
        case 0xc2: case 0xd2: case 0xca: case 0xda: case 0xc3: { //JP cond u16
            u16 addr = bus.next_word(regs.pc, regs.pc);
            if(cond(opcode)) {
                regs.pc = addr;
                cycles += 4;
        ...
        break;
        case 0x18:                                    //JR i8
        regs.pc += (i8)bus.next_byte(regs.pc, regs.pc);
        break;
        case 0x20: case 0x30: case 0x28: case 0x38: { //JR cond i8
            i8 offset = (i8)bus.next_byte(regs.pc, regs.pc);
            if(cond(opcode)) {
                regs.pc += offset;
                cycles += 4;
            }
        ...
        break;
        case 0xcb: {
            u8 cbop = bus.next_byte(regs.pc, regs.pc);
...
u8 Cpu::read_r8(u8 bits) {
    switch(bits) {
        ...
        case 6: return bus.read_byte(regs.hl);
...
void Cpu::write_r8(u8 bits, u8 val) {
    switch(bits) {
        ...
        case 6: bus.write_byte(regs.hl, val); break;
    ...
    if(int_mask) {
        ...
        if(ime) {
            if((bus.mem.ie & 1) && (bus.mem.io.intf & 1)) {
                ...
                ime = false;
                cycles += 20;
            } else if(bit(bus.mem.ie, 1) && bit(bus.mem.io.intf, 1)) {
                bus.mem.io.intf &= ~2;
                push(regs.pc);
                regs.pc = 0x48;
                ime = false;
                cycles += 20;
            } else if(bit(bus.mem.ie, 2) && bit(bus.mem.io.intf, 2)) {
                bus.mem.io.intf &= ~4;
                push(regs.pc);
                regs.pc = 0x50;
                ime = false;
                cycles += 20;
            } else if(bit(bus.mem.ie, 5) && bit(bus.mem.io.intf, 3)) {
                bus.mem.io.intf &= ~8;
                push(regs.pc);
                regs.pc = 0x58;
                ime = false;
                cycles += 20;
            } else if(bit(bus.mem.ie, 4) && bit(bus.mem.io.intf, 4)) {
                bus.mem.io.intf &= ~16;
                push(regs.pc);
                regs.pc = 0x60;
                ime = false;
