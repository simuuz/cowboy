...
void Cpu::Execute(byte opcode)
{
  ...
  case 0xe0:  // LD (FF00 + byte), A
    bus->WriteByte(timestamp, scheduler, 0xff00 + bus->NextByte(regs.pc, regs.pc), regs.a);
    break;
  case 0xf0:  // LD A, (FF00 + byte)
    regs.a = bus->ReadByte(0xff00 + bus->NextByte(regs.pc, regs.pc));
    break;
  case 0xe2:  // LD (FF00 + C), A
    bus->WriteByte(timestamp, scheduler, 0xff00 + regs.c, regs.a);
    break;
  ...
  case 0x08:  // LD (half), REGS.SP
    bus->WriteHalf(timestamp, scheduler, bus->NextHalf(regs.pc, regs.pc), regs.sp);
    break;
  ...
  break;
  case 0xea:  // LD (half), A
    bus->WriteByte(timestamp, scheduler, bus->NextHalf(regs.pc, regs.pc), regs.a);
    break;
  case 0xfa:  // LD A, (half)
